...
import { checkUploadStatus, uploadImage, resolveUploadUrl } from '../../services/uploadService';
...
const ImageUploadField = ({
  ...
  const [storageReady, setStorageReady] = useState(null); // null = not checked yet
  ...
  /* Check storage status lazily — only when user tries to upload */
  const checkStorageAndProceed = useCallback(async (file) => {
    ...
    // Storage is ready — proceed with crop
    setError('');
  ...
  const handleCropDone = async (blob) => {
    ...
    try {
      ...
      // Pass current value as replaceUrl to auto-delete old file
      const result = await uploadImage(file, folder, fileName, value || '');
      if (result?.url) {
        onChange(result.url);
  ...
  // Resolve the display URL using parseImageUrl for full compatibility
  // (handles Drive URLs, local /uploads/ paths, Cloudinary, etc.)
  const displayUrl = value ? parseImageUrl(value) : '';

  return (
    <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
      {label && <span className="block text-sm font-medium text-slate-700">{label} {required && <span className="text-rose-500">*</span>}</span>}

      {/* Upload drop zone — ONLY this area triggers file picker */}
      ...
      {displayUrl && (
        <div className="relative inline-block">
          <img src={displayUrl} alt="Preview" className="w-16 h-16 object-cover rounded-lg border border-slate-200 shadow-sm"
